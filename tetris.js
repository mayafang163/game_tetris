var game={
  ...
  randomShape(){//随机生成图形
    ...
    switch(Math.floor(Math.random()*3)){//判断r
      case 0: return new O();//是0，就返回一个新的O
      case 1: return new I();//是1，就返回一个新的I
      case 2: return new T();//是2，就返回一个新的T
    }
  },
  ...
  canRight(){ //判断能否右移
    ...
    for( var i=0;i<this.shape.cells.length;i++){
      //将当前cell另存为
      var cell=this.shape.cells[i];
      //如果cell的c等于CN或wall中cell右侧不是undefined
  ...
  canDown(){ //判断能否下落
    ...
    for(var i=0;i<this.shape.cells.length;i++){
      var cell=this.shape.cells[i];
      if(cell.r==this.RN-1) return false;//如果cell的r等于RN-1,就返回false
      //否则，如果wall中cell的下方位置不为undefined，就返回false
      else if(this.wall[cell.r+1][cell.c]!==undefined) return false;
    }//(遍历结束)
    return true;//就返回true
  },  
  ...
  deleteRow(r){ //删除第r行
    ...
    for(;r>=0;r--){
      ...
      if(this.wall[r-2].join("")=="")//如果wall中r-2行是空行
        break;//就退出循环
    }
  },
  ...
  paint(){ //重绘一切
    //先清除所有img
    var reg=/<img [^>]+>/g; // 可避免重复layout
    this.pg.innerHTML=this.pg.innerHTML.replace(reg,""); //改字符串，不会layout
    this.paintShape();//再重绘
    this.paintWall();
    this.paintNext();
    this.paintScore();
    ...
    for(var r=this.RN-1;r>=0;r--){
      ...
      if(this.wall[r].join("")=="") break;
        ...
        for(var c=0;c<this.CN;c++)
          ...
          if(this.wall[r][c]!==undefined)
            this.paintCell(this.wall[r][c],frag);//绘制wall中当前格
    }//(遍历结束)
    this.pg.appendChild(frag);//将frag追加到pg中
  },  
  paintCell(cell,frag){
    var img=new Image;//新建一个img
    img.style.width=this.CSIZE;//设置img的宽为CSIZE
